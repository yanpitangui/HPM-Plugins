//===== Hercules Plugin ======================================
//= bNoSingleMagic (from Epoque Expansion Pack)
//===== By: ==================================================
//= Yan Pitangui
//===== Current Version: =====================================
//= 0.1
...
struct player_data {
	int single_magic_resist;
};

int SINGLE_MAGIC_RESIST = 0;

int pc_bonus_pre(struct map_session_data** sd, int *type, int *val) {
	if (*sd == NULL )
		return 0;
	if ( *type == SINGLE_MAGIC_RESIST) {
		struct player_data *ssd = getFromMSD( *sd, 0 );
		if ( ssd == NULL ) {
			CREATE( ssd, struct player_data, 1 );
			ssd->single_magic_resist = cap_value(*val, 0, 100);
			addToMSD(*sd, ssd, 0, true);
		}
		else {
			ssd->single_magic_resist = cap_value(*val + ssd->single_magic_resist, 0, 100);
		}
		hookStop();
	}
	return 0;
...
int64 battle_calc_damage_post(int64 retVal___, struct block_list* src, struct block_list* bl, struct Damage* d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	if (flag & BF_MAGIC) {
		...
		struct player_data* ssd = getFromMSD(sd, 0);
		if (ssd && skill->get_casttype(skill_id) == CAST_DAMAGE && ssd->single_magic_resist != 0) {
			retVal___ -= retVal___ * ssd->single_magic_resist / 100;
		}
	}
	return retVal___;
}
...
HPExport void plugin_init (void) {
	SINGLE_MAGIC_RESIST = map->get_new_bonus_id();
	script->set_constant("bNoSingleMagic", SINGLE_MAGIC_RESIST, false, false );
